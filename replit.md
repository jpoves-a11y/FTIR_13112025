...
## Recent Changes
- 2025-11-13: **CRITICAL FIX - Eliminated Double Normalization in Deconvolution Workflow**
  - **Problem Identified**: Previous workflow normalized spectrum BEFORE deconvolution, then divided again by reference band area during concentration calculation, causing double normalization
  - **Dual-Pipeline Solution Implemented**:
    * **Calculation Pipeline** (RAW data): All curve fitting, Simpson integration, and concentration calculations use ORIGINAL data (not normalized)
    * **Display Pipeline** (normalized data): Visualization uses data scaled by normalizationFactor = 1/referenceBandArea
  - **Key Changes**:
    - `performDeconvolution()`: Eliminated pre-fitting normalization, now works on baseline-corrected ORIGINAL data
    - `processDepthDeconvolution()`: Same fix applied to Multiple Deconvolution mode
    - `calculateSpeciesAreasWithSimpson()`: Now reads RAW traces from `deconvolutionResults.calculation.rawIndividualTraces`
    - Dual storage structure:
      * `deconvolutionResults.calculation` = {rawOriginalSpectrum, rawFittedTrace, rawIndividualTraces}
      * `deconvolutionResults.display` = {normalizedOriginalSpectrum, normalizedFittedTrace, normalizedIndividualTraces}
  - **Concentration Workflow** (CORRECT - Single Normalization):
    1. Original Spectrum → Extract Region → Baseline Correction → RAW Data
    2. Calculate referenceBandArea (1980-2100 or 1330-1400 cm⁻¹) ONCE
    3. Curve Fitting (Adam optimization) on RAW Data
    4. Generate RAW traces → Store in calculation namespace
    5. Simpson Integration on RAW traces → simpsonArea_RAW
    6. **Concentration = (simpsonArea_RAW / (ε × referenceBandArea)) × 1000** ← SINGLE normalization
    7. For display: Scale RAW traces × normalizationFactor → Store in display namespace
  - **Validation**:
    - ✅ Architect PASS: "Individual and multiple deconvolution pipelines now consume raw spectra for all fitting and quantification stages, applying reference-band normalization exactly once"
    - ✅ Concentrations between Individual and Multiple modes are identical for same depth
    - ✅ Plotly graphs show normalized data (espectro/AUC banda referencia)
    - ✅ All calculations use original data (no pre-normalization)
  - **Impact**: Concentrations are now mathematically correct, with normalization applied only once in the Lambert-Beer equation

- 2025-11-13: **Dual Reference Band Selection for Deconvolution Analysis**
  - **Major Enhancement**: Replaced fixed reference band input with selectable options
  - **Two reference band options** now available in both Individual and Multiple Deconvolution:
    * **2020 cm⁻¹ (1980-2100 cm⁻¹)** - Twisting of CH₂ groups (Amorphous + Crystalline region)
    * **1370 cm⁻¹ (1330-1400 cm⁻¹)** - Wagging of CH₂ groups (Amorphous region only)
  - **UI Changes**:
    - Replaced numeric input field with radio buttons for band selection
    - Added scientific descriptions for each reference band option
    - Identical controls in both Individual (lines 2658-2674) and Multiple (lines 2860-2877) modes
    - Default selection: 2020 cm⁻¹ band
  - **Code Updates**:
    - `calculateReferenceBandArea()` (lines 11373-11430): Detects selected band and applies correct range (1980-2100 or 1330-1400)
    - `calculateReferenceBandAreaForDepth()` (lines 11914-11956): Accepts reference band parameter for Multiple mode
    - `runMultipleDeconvolution()` (lines 11785-11791): Reads selected band from UI controls
    - Automatic baseline correction and Simpson's rule integration preserved for both bands
  - **Consistency Verified**: Both deconvolution modes use identical reference band when selected
  - **Text Updates**: Removed hard-coded "2000-2040 cm⁻¹" references, replaced with generic "Reference Area"
  - **Scientific Rationale**:
    - 2020 band: Suitable for total thickness normalization (includes both phases)
    - 1370 band: Specific to amorphous phase, useful for phase-specific analysis
  - Architect reviewed and approved all changes
  - Application tested and verified working correctly

- 2025-11-13: **Fresh GitHub Import - Replit Environment Setup**
